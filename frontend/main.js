...
function bindActions() {
  ...
  $('openInteractionsInMy')?.addEventListener('click', async () => {
    ...
    const renderInteractionGroup = (title, list, kind) => `
      <div class="interaction-group timeline-group ${kind}">
        <div class="interaction-group-title">${title}</div>
        <div class="nearby-list interaction-records">${list.map(it => `<div class="nearby-row interaction-row ${kind}"><div class="interaction-rail"><span class="interaction-dot ${kind}"></span></div><div class="nearby-main interaction-main">${renderPersonSnippet({ nickname: kind === 'sent' ? (it.toNickname || ('用户'+it.toUserId)) : (it.fromNickname || ('用户'+it.fromUserId)), roleName: kind === 'sent' ? '我发出的互动' : '收到的互动', bio: it.message || '（无附言）', distanceKm: 0 }, { showDistance: false, promptText: false })}<div class="interaction-caption">${kind === 'sent' ? '你已发出邀请，等待对方回应。' : '对方给你发来了互动信号。'}</div></div><div class="record-side"><span class="record-status record-status-${String(it.status || '').toLowerCase()}">${it.status}</span>${it.toUserId ? `<button class="ghost-btn ghost-btn-soft btn-secondary record-jump-btn" data-record-user-id="${kind === 'sent' ? (it.toUserId || '') : (it.fromUserId || '')}" data-record-name="${kind === 'sent' ? (it.toNickname || '') : (it.fromNickname || '')}">查看对象</button>` : ''}</div></div>`).join('') || `<div class="empty-state compact-empty"><div class="small">暂无${title}</div></div>`}</div>
      </div>
    `
    setSubPanel(
      '互动记录',
